...
transform = transforms.Compose([transforms.Resize(30),
                                transforms.ToTensor(),
...
image_name = os.path.basename(opts.input).split(".")[0]
...
if opts.trainer == 'MUNIT':
    ...
    outputs = (outputs + 1) / 2.
    path = os.path.join(opts.output_folder, image_name + '_fake.jpg')
    vutils.save_image(outputs.data, path, padding=0, normalize=True)
else:
    pass
...
if not opts.output_only:
    ...
    vutils.save_image(image.data, os.path.join(opts.output_folder, image_name + '_real.jpg'), padding=0, normalize=True)
